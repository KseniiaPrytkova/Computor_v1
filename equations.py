...
def solve_quadratic_eq(data, flags):
    ...
    if flags['i']:
        result += ("\n(" + str(data[1]) +  " * " + str(data[1]) +  ") - " +  "(4 * " +  str(data[2]) + " * " + str(data[0]) + ") = \n")
        if flags['c'] and not flags['b']:
    ...
    if (d > 0):
        ...
        # x1 = ((-b + sqrt(D)) / (2a))
        if flags['i']:
            result += ("\nroot_1 = " + "(" + str(-data[1]) + " + " + str(comp_sqrt(d)) + ")" + " / " + "(" + "2" + " * " + str(data[0]) + ") = \n")
        root_1 = ((-data[1] - comp_sqrt(d)) / (2 * data[2]))
        result += print_the_bajskorv(root_1, flags)
        # x2 = ((-b - sqrt(D)) / (2a))
        if flags['i']:
            result += ("\nroot_2 = " + "(" + str(-data[1]) + " - " + str(comp_sqrt(d)) + ")" + " / " + "(" + "2" + " * " + str(data[0]) + ") = \n")
        ...
        # x1 = x2
        if flags['i']:
            result += ("\nroot_1 = root_2 = " + "(" + str(-data[1]) + " + " + str(comp_sqrt(d)) + ")" + " / " + "(" + "2" + " * " + str(data[0]) + ") = \n")
        root_1 = ((-data[1] - comp_sqrt(d)) / (2 * data[2]))
        result += print_the_bajskorv(root_1, flags)
    elif (d < 0):
